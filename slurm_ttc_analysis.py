...
sample = '100NP_water_2'     # sample name
...
q = 4.0e-10 * np.pi / wavelength * np.sin(0.5*tth) # q values in 1/Å
print(q)
...
pool = Pool(32)
ttc = pool.map(partial(worker, roi_img=roi_img, exp_time=exp_time, downsample=downsample), enumerate(scans)) # calculate the ttc
